  ...
  ajaxBase.interceptors.request.use(config => {
    ...
    switch (config.url) {
      ...
      case '/member/auth/signature/login':
        ...
        config.headers.Authorization = 'Bearer ' + window.localStorage.getItem('bet_client_access_token')
        break
      default:
        console.log('Token使用(bet): bet user')
        config.headers.Authorization = 'Bearer ' + window.localStorage.getItem('bet_user_access_token')
        break
    }
