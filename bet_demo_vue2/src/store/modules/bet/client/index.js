...
export default {
  ...
  actions: {
    ready (context) {
      console.log('Vues-store使用: model(storeBetClient).action.ready()')
    },
    clientAuth (context) {
      ...
      //   return
      // }
      ajaxBetClient.getToken().then(function (result) {
        // console.log(result)
        if (result.status === 200 && result.request.readyState === 4) {
          const apiData = result.data
          if (apiData !== null && apiData.success === true) {
            window.localStorage.setItem('bet_client_access_token', apiData.data.access_token)
            // 執行本地指令
